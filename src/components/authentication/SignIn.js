...
const ModalContainer = styled.div`
  background: ${(props) => (props.isDarkMode ? "#2c2c2c" : "red")};
  width: 100%;
  max-width: 380px;
  margin: 10px;
  padding: 2rem 1.5rem;
...
const Line = styled.span`
  flex: 1;
  height: 1px;
  background: ${(props) => (props.isDarkMode ? "#123" : "#ddd")};
`;
...
const SignInModal = ({ show, closeSignInModal }) => {
  ...
  const handleSignIn = () => {
    ...
    if (!email) {
      ...
      setEmailError("Enter a valid email (e.g., bhaveshmodani@gmail.com).");
      isValid = false;
    } else {
      setEmailError("");
    }
    ...
    if (isValid) {
      IoAlertCircle("Sign In Successful!");
